...
class SpatialPD:
    def __init__(self, grid_size=99, t_factor=1.6, density=0.5):
    ...
    def _plot_results(self, filename):
        ...
        ax1.set_title(f"Stan końcowy (c={self.t_factor:.4f})")
        ax1.axis("off")

        t = range(len(self.evolution["coop"]))
        ax2.set_title("Ewolucja strategii agentów w czasie")
        ...
        ax2.set_ylabel("Odsetek agentów poszczególnych strategii")
        ax2.legend()
        plt.tight_layout()
    ...
    def plot_phase_diagram(self, filename="phase_trajectory.png"):
        ...
        fc_history = np.array(self.evolution["coop"]) / total_cells
        fd_history = np.array(self.evolution["defect"]) / total_cells
        ...
        if points > 1:
            for i in range(points - 1):
                plt.plot(
                    fd_history[i : i + 2],
                    fc_history[i : i + 2],
                    color=plt.cm.jet(i / points),
                    lw=2,
                )

            plt.plot(fd_history[0], fc_history[0], "go", label="Start", markersize=1)
        else:
            plt.plot(fd_history, fc_history, "bo")

        plt.xlabel("Ułamek Zdrajców (f_d)", fontsize=14)
        plt.ylabel("Ułamek Kooperantów (f_c)", fontsize=14)
        plt.title(f"Diagram Fazowy Trajektorii (b={self.t_factor})", fontsize=16)
        plt.grid(True)
        plt.legend()
...
if __name__ == "__main__":
    try:
        t_fac = float(os.environ.get("T_FACTOR_VALUE", "1.9"))
    ...
    for run_idx in range(n_runs):
        sim = SpatialPD(grid_size=100, t_factor=t_fac, density=0.5)
        is_first = run_idx == 0
